...
import {
  ...
  ProductType,
} from "@/src/app/_lib/types/ProductType";
...
const CarouselCard = ({ product }: { product: ProductType }) => {
  const attributes: ProductAttributeType =
    product.attributes || ({} as ProductAttributeType);
    ...
    <Card className="h-[400px] flex flex-col border-border/50 bg-card/50 backdrop-blur-sm hover:border-primary/50 transition-all hover:shadow-2xl hover:shadow-primary/5">
      <CardHeader>
        <CardTitle className="flex items-center gap-4">
          ...
          <h3 className="text-xl text-[#0d4c92] font-black truncate">
            {attributes.title}
          </h3>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-[#2b619fce]  font-medium leading-relaxed line-clamp-6">
